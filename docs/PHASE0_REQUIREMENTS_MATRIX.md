...
| 9 | Append-only hash-chained ledger with daily seal | tested | `packages/core/src/ledger/`, ledger tests (trigger blocks UPDATE/DELETE; chain detects tampering even after triggers are dropped; seal mismatch throws); CLI drill `verify-chain` ok over 9 events. **The "daily" half was overstated until 2026-09-07**: sealing existed only as the `seal` CLI command, so a deployed `serve` container never sealed anything. Now a scheduled `seal_ledger` job seals every unsealed day up to yesterday — `packages/core/test/serve-seal-job.test.ts` (12 tests, including catch-up after downtime). Found by Codex review on PR #8, not by this matrix, which is why a row citing only unit tests of a mechanism is weaker evidence than one citing the deployed path |
