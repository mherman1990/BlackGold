...
| 7 | CFTC COT adapter with release schedule | tested | `adapters/cftc-cot.ts`, `adapters-cot.test.ts`; dataset ids marked UNVERIFIED (CR-26) |
...
| 11 | Date-effective entity map | tested | `market/entity-map.ts`, `market-entity-map.test.ts`, symbol-change fixture |
...
| 14 | Experiment registry: freeze, viewed, once-only holdout, promotion refusal | tested | `research/registry.ts` (DB triggers freeze definitions and log holdout once), `research-registry.test.ts` |
