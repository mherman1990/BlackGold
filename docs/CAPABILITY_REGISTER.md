...
| CR-17 | `${APP_DATA_DIR}` is the current umbrelOS app-data variable; `exports.sh` semantics; whether `manifestVersion` > 1 is accepted | not in template README | - | UNVERIFIED | Phase 0: inspect `getumbrel/umbrel-apps` examples and umbrelOS source | Phase 0 |
...
| CR-20 | GitHub Actions `docker/build-push-action` supports `linux/amd64,linux/arm64` via QEMU/buildx; GHCR anonymous pull for public packages | not fetched | - | UNVERIFIED (well-established but must be checked) | Phase 0 CI dry run | Phase 0 |
| CR-21 | Exchange calendar data for NYSE holidays and early closes available via an npm package or NYSE published schedule | not fetched | - | UNVERIFIED | Phase 0: choose library, verify 2026–2027 schedule against NYSE page | Phase 0 |
