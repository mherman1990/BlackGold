...
export function deterministicTargetBook(charter: Charter, deps: DecisionEngineDeps, decisionAt: UtcInstant, held: ReadonlySet<string>): ArmTargetBook {
  ...
  return { arm: "B1_DETERMINISTIC", decisionAt, anchorSession: fs.anchorSession, targetWeights: sortedByEntity(targets.weights), cashWeight: targets.cashWeight, labels };
}
