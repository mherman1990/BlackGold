...
- **Phase 3 is in progress** on `claude/phase-03-analyst` ([PR #22](https://github.com/mherman1990/BlackGold/pull/22), draft), authorized by D-41. The provider-agnostic analyst pipeline and its full safety surface are built and tested with a stub adapter under `packages/core/src/research/{assessment,packet}.ts` and `packages/core/src/model/{adapter,assess,overlay,manifest}.ts`; the T-05 gate is `test/policy/no-llm-in-sizing.test.ts`. **Deferred to a follow-up PR / Phase 5** (see `docs/PHASE3_REQUIREMENTS_MATRIX.md`): the real Anthropic adapter + the POST egress change (`live-disabled.test.ts` forbids the SDK and `data/http.ts` is POST-forbidden today) + live CR-11/12/13 re-verification, which needs an API key and Matt's review of the egress change; call/budget persistence; and C1/D1 backtest wiring. To resume: read the matrix, then either continue the deferred items or await Matt's review of PR #22.
