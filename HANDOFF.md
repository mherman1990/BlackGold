...
- `research-backtest.test.ts` and `research-report.test.ts` declare `vi.setConfig({ testTimeout: 30_000 })`. Vitest's 5-second default is not a realistic ceiling for a multi-decision backtest, and leaving it there let the same commit pass on one CI runner and time out on another. If a new suite runs a backtest, give it the same explicit budget rather than trusting the default. Both suites also share one read-only fixture market across their tests; `runBacktest` never writes, so that cannot couple them.
