...
- **Phase 4, second PR (D-44): the risk halt-state machine is built.** `packages/core/src/risk/halt.ts` (`evaluateHaltState`) returns `NORMAL`/`HALT_NEW_RISK`/`HOLD_ONLY`, never `EMERGENCY_FLATTEN_AUTHORIZED`; drawdown, daily loss, stale input, expired auth, unknown state, and severe incident fail closed to `HALT_NEW_RISK` (or `HOLD_ONLY` deeper); escalation is automatic, relaxation needs an owner re-arm an active fault still overrides. Sizing (`strategy/construct.ts`) and the risk-policy schema (`RiskConfigSchema`, `config/examples/risk.yaml`) already existed. **`risk.yaml` is still unapproved:** approving it is OD-3, an act no autonomy reaches, so `approvedBy` stays `null` and the engine consumes an unapproved policy — nothing registers or runs for real until Matt resolves OD-3. Still to come in Phase 4 (bounded PRs, gated on OD-3 / D-12 where relevant): exposure flags + look-through, the restricted list + compliance engine, the risk-limit/caps engine with reason codes, and wiring the halt state into the decision/gateway loop (Phase 5).
