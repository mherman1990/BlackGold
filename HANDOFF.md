...
- **Phase 3 completion** on `claude/phase-03-completion` (built 2026-09-07): call archiving + budget persistence (migration `0007` `model_calls`; `packages/core/src/research/model-call-log.ts`) and the analyst wiring (`packages/core/src/analyst/run-analyst.ts` engine; the `research analyst` CLI command in `main.ts`; `readAnthropicApiKey` in `config/load.ts`). This is the wiring layer above: the key is read only in `config/load.ts` and kept off `AppConfig`. To run once a key is on the Pi: `research analyst --manifest <model-manifest.yaml> --model <id> --candidate <SYMBOL> --at <iso> --sources <sourceId[:entityId],...> [--factors <csv>]`; it seals a point-in-time packet, assesses, and archives the redacted call. **Still open:** the live CR-12/CR-13 run (needs the key + a real call), C1/D1 backtest wiring (Phase 5, review-gated), and a deterministic factor classifier (Phase 4; until then `factorsTouched` must match the `--factors` set the operator passes).
