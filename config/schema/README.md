...
| `theme-membership.yaml` | `theme-membership.schema.json` | Owner-authored issuer -> restricted-theme membership plus the two look-through policy values (aggregate admissibility threshold as a fraction of ETF NAV, ALPHA_CHARTER.md section 2.2; holdings freshness limit in days). Consumed by `compliance/theme-membership.ts` to build the shadow decision's look-through resolver; the tracked example is fake, like the restricted list. |
