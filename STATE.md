...
All of Phase 4 (factor classifier, halt-state machine, risk-limit engine, compliance engine) and the first Phase 5 slice (the deterministic decision gate, PR #35) are on `main` and green; the owner has signed the charter and `risk.yaml`. The remaining Phase 5 pieces are gated on Matt's inputs (data credentials, the real restricted list, paper broker keys) and are not started autonomously without them. The one live CR-12/CR-13 analyst run still needs a key on the Pi. The next steps are Matt's, in rough order of what unblocks the most:
