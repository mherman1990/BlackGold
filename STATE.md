...
**Deferred (own follow-up PR / Phase 5), by design — see `docs/PHASE3_REQUIREMENTS_MATRIX.md`:** the real Anthropic adapter + the POST egress change + live CR-11/12/13 re-verification (needs an API key and a review of the egress change); persistence of the model-call record and running budget; and wiring C1/D1 into the decision loop (a real C1/D1 run is prospective, Phase 5). No live mode, broker credential, or provider egress path is added. No LLM output can set a size, choose an account, or form an order.
